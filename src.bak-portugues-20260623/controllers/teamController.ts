...
export async function grantSupportAccess(req: Request, res: Response) {
  try {
    ...
    const reason = String(req.body?.reason || "Suporte tecnico autorizado pelo cliente").slice(0, 240);
...
export async function getPrivacyReport(req: Request, res: Response) {
  try {
    ...
    return res.json({
      ...
      supportLogs,
      notes: [
        "Este relatorio nao inclui conteudo de conversas, mensagens, nomes de leads ou dados sensiveis.",
