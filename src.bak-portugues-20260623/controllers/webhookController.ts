...
function isAudioMessage(raw: any) {
  return Boolean(raw?.message?.audioMessage);
}
    ...
    for (const entry of entries) {
      ...
      for (const change of changes) {
        ...
        for (const message of messages) {
          ...
          if (message?.audio) {
            try { text = await transcribeMetaAudio(instance, message); } catch (error) { console.error("Erro ao transcrever audio Meta:", error); text = "[Audio recebido, mas nao foi possivel transcrever automaticamente]"; }
          }
          if (!text) continue;
