...
function appendTags(current: string, tags: string[]) {
  const set = new Set(
    String(current || "")
      .split(",")
      .map((item) => item.trim())
...
export async function listConversations(req: Request, res: Response) {
  try {
    ...
    const visibleData = alertOnly === "true"
      ? data.filter((conversation) => !isResolvedLead(conversation.lead))
      : data;
...
export async function markHumanHandoff(req: Request, res: Response) {
  try {
    const companyId = req.auth!.companyId;
    const id = getId(req.params.id);
    ...
    const reasonText = String(reason || "Encaminhado para atendimento humano pela Laura.").trim();
    const summaryText = String(summary || "").trim();
    const alertNote = [
      "[ALERTA HUMANO] " + new Date().toLocaleString("pt-BR"),
      reasonText,
      summaryText ? "Resumo: " + summaryText : "",
    ].filter(Boolean).join("\n");
    ...
    const updated = await prisma.conversation.update({
      ...
      data: {
        isHot: true,
        tags: appendTags(conversation.tags, ["alerta_humano", "urgente", "advogado_agir"]),
        notes: [conversation.notes, alertNote].filter(Boolean).join("\n\n"),
    ...
    if (conversation.leadId) {
      await prisma.lead.updateMany({
        ...
        data: {
          stage: "urgente",
          qualificationNotes: [conversation.lead?.qualificationNotes, alertNote].filter(Boolean).join("\n\n"),
        },
      });
    }
    ...
    console.error("Erro markHumanHandoff:", error);
    return res.status(500).json({ error: "Erro ao criar alerta humano" });
  }
}
