...
const RESOLVED_LEAD_STATUSES = ["qualificado", "consulta_marcada", "contratado", "perdido", "ruim"];
const RESOLVED_LEAD_STAGES = ["consulta marcada", "contratado", "fechado", "perdido"];
...
function isResolvedLead(lead?: { qualificationStatus?: string | null; stage?: string | null } | null) {
  if (!lead) return false;
  const status = String(lead.qualificationStatus || "").toLowerCase();
  const stage = String(lead.stage || "").toLowerCase();
  return RESOLVED_LEAD_STATUSES.includes(status) || RESOLVED_LEAD_STAGES.includes(stage);
}
...
export async function getDashboardMetrics(req: Request, res: Response) {
  try {
    ...
    const leadMonthWhere = { companyId, createdAt: { gte: since } };
    const conversationMonthWhere = { companyId, createdAt: { gte: since } };
    ...
    const [
      ...
      prisma.conversation.count({ where: conversationMonthWhere }),
      prisma.conversation.count({ where: { ...conversationMonthWhere, unreadCount: { gt: 0 } } }),
      prisma.conversation.count({ where: { ...conversationMonthWhere, isHot: true } }),
      prisma.faqRule.count({ where: { companyId, active: true } }),
      prisma.lead.count({ where: leadMonthWhere }),
      prisma.lead.count({ where: { ...leadMonthWhere, stage: { in: ["Novo lead", "novo"] } } }),
      prisma.lead.count({ where: { ...leadMonthWhere, stage: { in: CLOSED_STAGE_NAMES } } }),
      prisma.lead.count({ where: { ...leadMonthWhere, stage: { in: LOST_STAGE_NAMES } } }),
      prisma.lead.count({
        where: { ...leadMonthWhere, stage: { in: [...NEW_STAGE_NAMES.slice(1), ...OLD_NEGOTIATION_STAGES] } },
      }),
    ...
    const humanAlertItems = await prisma.conversation.findMany({
      where: {
        companyId,
        isArchived: false,
        OR: [
          { isHot: true },
          { tags: { contains: "alerta_humano" } },
          { tags: { contains: "advogado_agir" } },
          { tags: { contains: "urgente" } },
          { followUpStatus: "scheduled", followUpAt: { lte: new Date() }, followUpSentAt: null },
        ],
      },
      select: {
        id: true,
    ...
    const actionableHumanAlertItems = humanAlertItems.filter((item) => !isResolvedLead(item.lead));
    ...
    return res.json({
      ...
      totalConversations,
      unreadConversations,
      hotLeads: actionableHumanAlertItems.length,
      humanAlerts: actionableHumanAlertItems.length,
      humanAlertItems: actionableHumanAlertItems.map((item) => ({
        id: item.id,
        leadId: item.lead?.id || null,
        name: item.contactName || item.phone,
        phone: item.phone,
        lastMessageText: item.lastMessageText || "Sem mensagem recente",
        lastMessageAt: item.lastMessageAt || item.updatedAt,
        tags: item.tags,
        source: item.lead?.campaign || item.lead?.source || item.lead?.keyword || "Origem nao informada",
        stage: item.lead?.stage || "urgente",
        score: item.lead?.leadScore || null,
        urgency: item.lead?.urgency || null,
      })),
      activeRules,
      totalLeads,
      newLeads,
      ...
      insights: [
        ...
        dueFollowUps > 0
          ...
          : "Follow-ups em dia.",
        aiMessages > 0
          ? `IA participou de ${aiMessages} respostas neste mes.`
